...
void FreeToTable(struct PaiXing *px,int ToTable[],int &Num,int Line,int Hands[])    //出牌
...
void mainsort(int cards[])//将totable一四张，三张，两张，一张排序，方便确定出牌类型，并比较大小
{
	int card_num = 0;/*牌的数量*/
	int i,j,k=0,l;  
	int sorted=0;  
...
void pokersort(int cards[])//将cards从小到大排序
{
	int i,j;
	int temp;//交换值的中间变量
...
void iTos(int iToTable[],char sCommandOut[])	//将iToTable[]中的数字转化为sCommandOut[]字符串
{
	int i;
	char t[4];
	if(iToTable[0]==-1)
...
void TakeOff(int iOnHand[],int iToTable[])	//将iToTable[]中的牌从iOnHand[]中剔除
{
	int i,j,k;
	for(i=0;iToTable[i]>=0;i++)
...
void GetDou(struct ddz * dp)					//版本信息
{
	int i;
...
void GetInf(struct ddz * dp)					//轮次信息处理函数:输入INFO 1/4 1/9 9 2450 输出OK INFO
{
	int a,b,c,d;
...
void GetDea(struct ddz * dp)		//底牌信息
{
	int i,j=0,t;
...
void GetLef(struct ddz * dp)			 //处理底牌信息
{
    int i,j=17;
...
void GetGam(struct ddz * dp)				//GAMEOVER 指令处理
{
...
void helpdz(int arrOnHand[],int arrOnTable[],int arrToTable[],int Hands[],struct PaiXing *px,int Line)//在arrToTable中返回所有可行序列-1间隔,-2结束
{
